...
@app.route("/venue/suspend/<int:venueId>", methods=["DELETE"])
...
@app.route("/venue/update/<int:venueId>", methods=["PUT"])
...
@app.route("/venue/getVenueById/<int:venueId>")
def get_venue_by_venueid(venueId):
    venue = Venue.query.get(venueId)
   
    if venue:
        return jsonify({"code": 200, "data": venue.to_dict()}), 200
    else:
        return jsonify({"code": 404, "message": "No venue with this ID present in DB."}), 404
    
@app.route("/venue/getVenueById123/<int:venueId>")
def get_venue_by_venueId():
    ...
    if venue1:
        ...
        if len(select)==1:
            ...
            return jsonify({ "code": 404, "message": "No venue with this venue present in DB." }), 404
    else:
        return jsonify({ "code": 404, "message": "No venue provided" }), 404
